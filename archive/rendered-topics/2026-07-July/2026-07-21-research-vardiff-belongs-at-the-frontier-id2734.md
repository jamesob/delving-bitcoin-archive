...
>  **Correction (2026-07-26)**: warioishere, Blitzpool's author, tells me the count-based controller I describe on the JD path was never IP-paired and never ran — it has since been removed. So the claim that Blitzpool deploys two matched difficulty algorithms at two points in its topology is false as deployed, and I retract it. The frontier argument doesn't rest on the dead witness; the deployed picture can be filled in properly once the live behavior is settled.
...
That is the frontier principle, deployed — but reached through **filtering**, not aggregation. *(Verified against the SV2 JD-client's own source: sub-target shares are rejected at the ordinary per-channel target check, one hop up, not forwarded — a clean instance of a signal-degrading boundary.)*
...
The counting argument says each telescope position has a *different* statistical character — a clean per-miner stream and a blended $N$-miner sum want different estimators. This is what filtering at the frontier looks like in the reference JD-client — see the correction at §7 regarding Blitzpool's deployed controllers. The reference SV2 pool does not: it runs the **same** difficulty algorithm on every channel it terminates. *(Verified from source.)*
...
gimballock | 2026-07-26 16:04:41 UTC | #2

I posted a small correction in section 7 after discussion with @warioishere, the second vardiff logic was never live but the logic of it, coincidentally, still follows from the theory presented. Furthermore wario and I are discussing plans to continue developing this theory.
